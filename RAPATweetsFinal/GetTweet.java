	...
	public void writeFile() {
		...
		try {
			...
			while (tweetIterator.hasNext()) {
				...
				String curTweet = tweetIterator.next().getText();
				if (curTweet.substring(0, 2).equals("RT")) {
					int x = curTweet.indexOf(":");
					curTweet = curTweet.substring(x+2);
				}
				
				writer.println(curTweet);
			}
			writer.close();
